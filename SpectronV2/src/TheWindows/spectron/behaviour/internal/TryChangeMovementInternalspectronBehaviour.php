...
final class TryChangeMovementInternalspectronBehaviour implements spectronBehaviour{
    ...
    private function checkObstaclesForJump(Player $player) : bool{
        ...
        $directions = [
            $front_dir, // Motion or front
            (new Vector3(-sin($rad), 0, cos($rad)))->normalize()->multiply(-1.0), // Behind
            (new Vector3(-sin($rad + M_PI_2), 0, cos($rad + M_PI_2)))->normalize()->multiply(1.0), // Left
            (new Vector3(-sin($rad - M_PI_2), 0, cos($rad - M_PI_2)))->normalize()->multiply(1.0) // Right
        ];
        foreach($directions as $dir){
            ...
            if($block instanceof Wheat || $block instanceof Sign || $block instanceof TallGrass || !$block->isSolid() && empty($block->getCollisionBoxes())){
                continue; // Skip non-solid, walk-through blocks
            }
            ...
            if($block->isSolid() || !empty($block->getCollisionBoxes())){
                if(!$above->isSolid() && empty($above->getCollisionBoxes())){
                    ...
                    $this->triggerJump($player, true); // Forward jump for 1-block obstacle
                    return true;
                }
    ...
    private function isBetterItem(Player $player, Item $item) : bool{
        ...
        if($item instanceof Armor){
            ...
            return true; // Pursue any valid armor item
        }
    ...
    private function getSwordDamage(Sword $sword) : float{
        return match($sword->getTypeId()){
            ...
            default => 4.0 // Fallback for custom swords
        };
    }
    ...
    private function setLookDirection(Player $player, Vector3 $target) : void{
        ...
        $pitch = 0; // Look straight ahead for walking
    ...
    private function updateRandomWalk(Player $player) : void{
        ...
        $speed = $distance < 1.0 ? 0.05 : ($this->data->movement_mode === 'walk' ? 0.12 : 0.15); // Slower speed for precision
        ...
        if($distance < 0.25){ // Reduced from 0.5 to ensure pickup
            $this->data->random_walk_target = null;
            $this->data->random_walk_ticks = 0;
            $this->data->target_item_id = null;
    ...
    public function tick(spectron $player) : void{
        ...
        // Check if random walk is disabled; if so, stop all movement
        if(!$this->plugin->isRandomWalkEnabled()){
            // Clear horizontal motion and movement-related data
            $this->data->motion = new Vector3(0, $this->data->motion->y, 0); // Preserve vertical motion for gravity
            $this->data->random_walk_target = null;
            $this->data->random_walk_ticks = 0;
            $this->data->target_item_id = null;
            $this->data->is_jumping = false;
        ...
        // Normal movement logic when random walk is enabled
        ...
        if(($this->data->needs_jump || !$this->data->is_pvp_active) && $this->checkObstaclesForJump($player_instance)){
            // Jump triggered within checkObstaclesForJump
        }
    ...
    public function onRespawn(spectron $player) : void{
        ...
        $this->data->jump_forward = false;
        $this->data->target_item_id = null;
        $this->data->motion = new Vector3(0, 0, 0); // Clear all motion on respawn
    ...
    public function updateConfig(array $config) : void{
        if(isset($config['random_walk_enabled'])){
            $this->data->is_pvp_active = !$config['random_walk_enabled']; // Disable PVP if random walk is disabled
